...
static log_module_level_t* log_level_stack = NULL;
static size_t log_stack_size = 0;  // Numero di istanze nello stack
...
static bool LOG_set_level(uint8_t tag, uint8_t level, const char *name) {
    ...
    // Increment the stack size and add the new instance
...
void LOG_initialize (bool enable) {
    ...
    ret_code |= LOG_set_level(MLX90614_DRV_LOG, LOG_ERROR_LEVEL, "MLX90614_DRV");
    ret_code |= LOG_set_level(MLX90614_PLUGIN_LOG, LOG_INFO_LEVEL, "MLX90614_PLUGIN");
    ret_code |= LOG_set_level(I2C_DRV_LOG, LOG_ERROR_LEVEL, "I2C_DRV");
