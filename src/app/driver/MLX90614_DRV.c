...
static eeprom_table_t eeprom_table;
static ram_table_t ram_table;
static mlx90614_drv_config_t config;

/* ************************************************************************** */
/// TODO: Local Methods
/* ************************************************************************** */

static void    MLX90614_DRV_powerup (void);
static void    MLX90614_DRV_powerdown (void);
static void    MLX90614_DRV_init (mlx90614_drv_config_t *config);
static void    MLX90614_DRV_config(mlx90614_drv_config_t *config);
static uint8_t MLX90614_DRV_SMB_pec(uint8_t* data, size_t length);
static bool    MLX90614_DRV_SMB_command (uint8_t command);
static bool    MLX90614_DRV_SMB_read (uint8_t command, uint16_t* value);
static bool    MLX90614_DRV_SMB_write (uint8_t command, uint16_t value);
static bool    MLX90614_DRV_SMB_read_EEPROM (uint8_t reg_addr, uint16_t* value);
static bool    MLX90614_DRV_SMB_read_RAM (uint8_t reg_addr, uint16_t* value);
static bool    MLX90614_DRV_SMB_write_EEPROM (uint8_t reg_addr, uint16_t value);
static bool    MLX90614_DRV_SMB_dumpEE (void);
static bool    MLX90614_DRV_SMB_dumpRAM (void);

static bool    MLX90614_DRV_SMB_Set_Register (uint16_t value, uint8_t address, uint16_t *p_eeprom_table);
static bool    MLX90614_DRV_SMB_Set_ConfigRegister1 (uint16_t mask, int shift, uint16_t value);
static bool    MLX90614_DRV_SMB_Set_PWMCTRL (uint16_t mask, int shift, uint16_t value);

static bool    MLX90614_DRV_SMB_Set_FIR (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_IIR (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_IRsensor (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_RepeatSensorTest (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_Tchannel (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_KsSign (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_Gain (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_Kt2Sign (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_SensorTest (uint16_t value);

static bool    MLX90614_DRV_SMB_Set_Emissivity (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_SlaveAddress (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_TOmax (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_TOmin (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_TAmax (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_TAmin (uint16_t value);

static bool    MLX90614_DRV_SMB_Set_PWM_mode (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_EN_PWM (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_PPODB (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_TRPWMB (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_PWMrepetition (uint16_t value);
static bool    MLX90614_DRV_SMB_Set_PWMperiod (uint16_t value);
...
static void MLX90614_DRV_config (mlx90614_drv_config_t *_config) {

    LOG_print_debug(MLX90614_DRV_LOG, "load user configuration");
    config.pwr_pin = _config->pwr_pin & 0xFF;
    ...
    config.emissivity = _config->emissivity % 100;    
    config.repeat_sensor_test = _config->repeat_sensor_test & 0x01;   
    config.t_channel = _config->t_channel & 0x03;
    config.ks_sign = _config->ks_sign & 0x01;
    config.gain = _config->gain & 0x07;
    config.kt2_sign = _config->kt2_sign & 0x01;
    config.sensor_test = _config->sensor_test & 0x01;    
    config.pwm_mode = _config->pwm_mode & 0x01;
    config.en_pwm = _config->en_pwm & 0x01;
    ...
    config.to_max = _config->to_max % 655;
    config.to_min = _config->to_min % 655;
    config.ta_max = _config->ta_max % 125;
    config.ta_min = _config->ta_min % 125;
}
...
static bool MLX90614_DRV_SMB_Set_Emissivity (uint16_t value) {
    ...
    // Convert value to expected uint format
    uint16_t emissivity = (uint16_t) (655.35 * value + 0.5);
        ...
        if (MLX90614_DRV_SMB_Set_Register(emissivity, SMB_EEPROM_EMISSIVITY_REG_ADDR, &eeprom_table.emissivity))
            if (MLX90614_DRV_SMB_Set_Register(eeprom_table.melexis_reserved_2[0], 0x0F, &eeprom_table.melexis_reserved_2[0]))
                if (MLX90614_DRV_SMB_command(0x61)) {
                    LOG_print_info(MLX90614_DRV_LOG, "set emissivity with value %d", value);
                    return true;
                }
	
    LOG_print_error(MLX90614_DRV_LOG, "set emissivity with value %d failed", value);
    return false;
}
...
/// @brief Write the EEPROM TO max register (temperature in Fahrenheit) (0 <> 600)
static bool MLX90614_DRV_SMB_Set_TOmax (uint16_t value) {
...
/// @brief Write the EEPROM TO min register (temperature in Fahrenheit) (0 <> 600)
static bool MLX90614_DRV_SMB_Set_TOmin (uint16_t value) {
...
/// @brief Write the EEPROM TA max in TA range register (temperature in Celsius) (0 <> 125)
static bool MLX90614_DRV_SMB_Set_TAmax (uint16_t value) {
	
    value = (uint16_t)((value + 38.2) * 1.5625);   
    value = value & 0x00FF;
...
/// @brief Write the EEPROM TA min in TA range register (temperature in Celsius) (0 <> 125)
static bool MLX90614_DRV_SMB_Set_TAmin (uint16_t value) {
